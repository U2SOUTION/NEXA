<!-- NetworkList.vue
  네트워크 목록 컴포넌트
  네트워크 요청 목록 표시 및 선택 기능
-->

<template>
  <div class="network-list-container">
    <q-scroll-area class="network-list-scroll-area">
      <!-- 로딩 상태 -->
      <div v-if="isLoading" class="loading-section q-pa-lg text-center">
        <q-spinner color="primary" size="3em" />
        <div class="q-mt-md text-caption">네트워크 요청을 불러오는 중...</div>
      </div>
      ...
      <q-list v-else separator>
        ...
        >
          ...
          <q-item-section>
            ...
            <q-item-label caption class="request-meta">
              ...
              <span v-if="request.size" class="q-ml-sm">{{ formatSize(request.size) }}</span>
        ...
        <div v-if="requests.length === 0" class="empty-section q-pa-lg text-center">
          <q-icon name="network_check" size="48px" color="grey-5" class="q-mb-md" />
          <div class="text-grey-7">네트워크 요청이 없습니다.</div>
        </div>
      </q-list>
    </q-scroll-area>
  </div>
