 ...
 * 패널 통합(Panels Integration): 기존에 포뮬레이터와 연결선만 있던 구조에 panels 배열을 추가했습니다.
 * 이로써 **"어떤 패널(데이터 원천)에서 시작해서, 어떤 포뮬레이터(가공)를 거쳐, 어떤 패널(결과)로 끝나는지"**에 대한 전체 흐름을 저장할 수 있습니다.
...
import { PanelSchema } from './panel' // [추가] 시각적 패널(조리대) 규격

/**
 * [ BlueprintSchema ]
 * NEXA 시스템의 '전체 로직 설계도'입니다.
 * 지구 어딘가에 존재하는 패널들과 그 안의 데이터 흐름을 한눈에 담는 지도 역할을 합니다.
 */
