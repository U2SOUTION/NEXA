<template>
  <div class="explorer-view-card column">
    ...
    <div v-else class="card-grid q-pa-sm">
      ...
      >
        ...
        <q-card-section class="q-py-xs">
          <div class="text-body2 text-weight-medium ellipsis" :title="item.original_name">
            {{ item.original_name || '이름 없음' }}
          </div>
          <div class="text-caption text-grey-7">{{ formatSize(item.file_size) }} · {{ item.domain }}</div>
        </q-card-section>
      </q-card>
    </div>
