  ...
  <q-page padding class="column no-wrap" style="padding: 10px">
    ...
    <div class="row q-col-gutter-md full-height" style="flex-grow: 1; min-height: calc(100vh - 120px)">
      ...
      <div class="col-xs-12 col-md-8 column" style="padding-top: 3px">
        <q-card class="step-form-container column items-center justify-center q-pa-md full-height">
          ...
          <div v-if="currentStep === 'form'" class="step-wrapper column items-center full-width q-mt-xl" style="width: 100%; max-width: 700px">
            <add-nexage-group-form v-if="selectedItemType === 'group'" @save="handleSaveGroup" @cancel="resetToTypeSelection" @go-prev-step="resetToTypeSelection" :parent-node="selectedNode" :is-editing="isEditing" :initial-data="isEditing ? selectedNode : null" />

            <add-nexage-form v-else-if="selectedItemType === 'board'" @save="handleSaveBoard" @cancel="resetToTypeSelection" @go-prev-step="resetToTypeSelection" :parent-node="selectedNode" :is-editing="isEditing" :initial-data="isEditing ? selectedNode : null" />
          </div>
        </q-card>
      </div>
      ...
      <div class="col-xs-12 col-md-4 column" v-if="$q.screen.gt.sm" style="padding-top: 3px; padding-left: 3px">
        <nexage-guide ref="boardGuideRef" :current-step="currentStep" :highlighted-node="highlightedNode" class="full-height guide-container" />
      </div>
    </div>
    ...
    <q-dialog v-model="showMobileGuide">
      <q-card style="min-width: 350px">
        ...
        <q-card-section class="q-pt-none">
          <nexage-guide :current-step="currentStep" :highlighted-node="highlightedNode" />
        </q-card-section>
      </q-card>
    </q-dialog>
  </q-page>
...
import NexageGuide from '../../components/NexageGuide.vue'
import AddNexageGroupForm from '../../components/AddNexageGroupForm.vue'
import AddNexageForm from '../../components/AddNexageForm.vue'
...
watch(
  ...
  (newSelection, oldSelection) => {
    console.log('NexageAdminPage: drawerSelectionForAdmin changed from', oldSelection, 'to:', newSelection)
