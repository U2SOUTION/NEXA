...
NexageAdminPage.vue에서 dashboardLayoutStore.selectedNodeIdForEditor 값이 있을 때, 이 NexageConfigEditor.vue를 표시하도록 하고 editingNodeId를 전달합니다. selectedNodeIdForEditor가 null이면 기존의 "최상위 항목 생성" UI를 표시합니다.
-->
...
import { useDashboardLayoutStore } from '@system/store/dashboardLayoutStore' // 대시보드 프리셋 목록 가져오기용
...
function safeNotify(options) {
  if ($q && typeof $q.notify === 'function') {
    ...
    console.warn('[NexageConfigEditor] Fallback: Used direct Notify.create()')
  } else {
    console.error('[NexageConfigEditor] $q.notify and Notify.create are not available. Message:', options.message)
    // 최후의 수단으로 alert 사용
...
function handleSaveChanges() {
  ...
  try {
    ...
    console.error('[NexageConfigEditor] Error saving node:', error)
