<template>
  <div class="nexia-left-nav column">
    <StandardLeftHeader title="NEXA NEXIA" subtitle="Channel & Chat Management" />
        ...
        <div class="panel-scroll-area">
          <div class="nexia-panel-padding">
            <q-expansion-item icon="sticky_note_2" label="메모" :default-opened="true">
              <div class="nexia-accordion-content">
                <div v-if="filteredMemos.length === 0" class="nexia-placeholder text-grey-6 text-caption">{{ memos.length === 0 ? '채팅에서 우클릭 → 메모로 추가' : '검색 결과 없음' }}</div>
    ...
    <q-menu v-model="contextMenuVisible" context-menu :position-x="contextMenuX" :position-y="contextMenuY" class="nexia-media-context-menu">
...
import { useNexiaChannels } from '../../composables/useNexiaChannels'
import { useNexiaAssets } from '../../composables/useNexiaAssets'
import { useNexiaLeftToolbar, type LeftMainTab } from '../../composables/useNexiaLeftToolbar'
import type { NexiaAssetItem, ExplorerFile } from '../../types/nexiaDomainTypes'
import type { Channel, Chat } from '../../types/nexiaDomainTypes'
import type { AssetCategory } from '../../composables/useNexiaAssets'
import { useNexiaUnifiedSearch, registerChannelsSync, SEARCH_TARGET_OPTIONS, CHAT_SEARCH_TARGET_OPTIONS, FILE_SORT_OPTIONS, FILE_CATEGORY_OPTIONS } from '../../composables/useNexiaUnifiedSearch'
import { useNexiaSettings, requestAttachToChat } from '../../composables/useNexiaSettings'
import { useNexiaExplorerSelection } from '../../composables/useNexiaExplorerSelection'
import { useNexiaMemos } from '../../composables/useNexiaMemos'
import { useNexiaInsertRequest } from '../../composables/useNexiaInsertRequest'
import { useNexiaMediaTab } from '../../composables/useNexiaMediaTab'
import { useFileSelection } from '@system/composables/useFileSelection'
import { showPanel } from '../../composables/useNexiaSplitLayout'

const { memos, loadMemos, removeMemo, moveMemoUp, moveMemoDown, getMemoPreview } = useNexiaMemos()
const { setSelectedFile } = useFileSelection()
const aiAssets = useNexiaAssets()
const { requestInsert, requestOpenEditor } = useNexiaInsertRequest()
const { onOpenMediaTab } = useNexiaMediaTab()
const { requestInjectToEditor, requestOpenInImageEditor, requestOpenInAudioEditor, requestOpenInVideoEditor, requestOpenInCodePanel } = useNexiaExplorerSelection()
...
function isCodeFile(file: NexiaAssetItem | { original_name?: string } | null) {
  if (!file?.original_name) return false
  const ext = (file.original_name || '').toLowerCase().match(/\.([a-z0-9]+)$/)?.[1] || ''
  return CODE_EXTENSIONS.includes(ext)
}

function getFileExt(file: NexiaAssetItem | { original_name?: string; file_path?: string } | null) {
  const name = (file?.original_name || (file as NexiaAssetItem)?.file_path || '').toLowerCase()
...
const projects = ref<ProjectItem[]>([
  { id: 'p1', name: 'AI 웹앱 개인작업', description: 'NEXA NEXIA 도메인 관련 실험', updatedAt: '2025-03-10' },
...
const contextMenuFile = ref<NexiaAssetItem | null>(null)
...
function openMediaContextMenu(evt: MouseEvent, f: NexiaAssetItem) {
  if (!f?.url) return
...
/** 템플릿에서 display* union( NexiaAssetItem | MediaPlaceholder ) 전달 시 타입 좁히기 */
...
function isImageForChat(file: (NexiaAssetItem & { file_type?: string; category?: string }) | null) {
...
function onDocumentClick(f: NexiaAssetItem) {
  if (!f?.url) return
...
/** 클릭: 뷰어에 단일 표시. md도 파서로 뷰어. 코드 파일(md 제외)만 Monaco. 삽입은 우클릭 "에디터에 넣기"로만. */
function onMediaItemClick(f: NexiaAssetItem, category: string) {
  if (!f?.url) return
  setSelectedFile(f as unknown as Record<string, unknown>)
...
type DisplayFileItem = NexiaAssetItem | MediaPlaceholder
...
const unifiedSearch = useNexiaUnifiedSearch()
...
const { pendingWebcamCapture, webcamFlipMode, webcamResolution, webcamFilterBrightness, webcamFilterContrast, webcamFilterSaturate, webcamFilterGrayscale, selectedModelCapabilities } = useNexiaSettings()
...
const { toolbarItems, toolbarLabel } = useNexiaLeftToolbar(toolbarCtx)
...
.nexia-left-nav {
  --nexia-selected-icon-color: var(--nexa-accent);
  ...
  .channel-item.channel-selected :deep(.q-icon) {
    color: var(--nexia-selected-icon-color);
  }
