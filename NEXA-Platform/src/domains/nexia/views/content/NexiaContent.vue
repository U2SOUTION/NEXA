<template>
  <div class="nexia-content column">
    <NexiaSplitLayout
...
import NexiaSplitLayout from './NexiaSplitLayout.vue'
...
const { onInsertRequest, onOpenEditorRequest } = useNexiaInsertRequest()
const { onInjectToEditor, onOpenInCodePanel } = useNexiaExplorerSelection()
...
async function injectMdToEditor(file) {
  ...
  try {
    ...
    console.error('[NexiaContent] MD fetch failed:', e)
...
async function injectTxtToEditor(file) {
  ...
  try {
    ...
    console.error('[NexiaContent] TXT fetch failed:', e)
...
async function injectCsvToEditor(file) {
  ...
  try {
    ...
    console.error('[NexiaContent] CSV fetch failed:', e)
...
async function injectDocxToEditor(file) {
  ...
  try {
    ...
    console.error('[NexiaContent] DOCX convert failed:', e)
...
async function injectXlsxToEditor(file) {
  ...
  try {
    ...
    console.error('[NexiaContent] XLSX convert failed:', e)
...
onMounted(() => {
  ...
  unregisterOpenInCodePanel = onOpenInCodePanel(async (file) => {
    ...
    try {
      ...
      console.error('[NexiaContent] code file fetch failed:', e)
    }
  })
})
