/**
 * useNexiaAssets - AI 도메인 파일 자산 관리
...
import type { NexiaAssetItem, UploadProgressItem } from '../types/nexiaDomainTypes'

const DOMAIN = 'ai'

export type AssetCategory = 'documents' | 'images' | 'audio' | 'video'

const documents = ref<NexiaAssetItem[]>([])
const images = ref<NexiaAssetItem[]>([])
const audio = ref<NexiaAssetItem[]>([])
const videos = ref<NexiaAssetItem[]>([])

/** 업로드 진행률 표시용 */
const uploadProgressFiles = ref<UploadProgressItem[]>([])
const showUploadProgress = ref(false)

export function useNexiaAssets(category: Ref<AssetCategory | null> | AssetCategory | null = null) {
  ...
  async function listFiles(cat: AssetCategory | null = null): Promise<NexiaAssetItem[]> {
    const base = getApiBaseUrl()
    const q = cat ? `?domain=${DOMAIN}&category=${cat}` : `?domain=${DOMAIN}`
    const res = await fetch(`${base}/files/list${q}`)
    const data = (await res.json()) as { items?: NexiaAssetItem[] } | NexiaAssetItem[]
  ...
  async function loadCategory(cat: AssetCategory | null): Promise<void> {
    try {
      ...
      console.error('[useNexiaAssets] loadCategory 실패:', cat, err instanceof Error ? err.message : String(err))
    }
  }
  ...
  function getTargetRef(cat: AssetCategory | null): Ref<NexiaAssetItem[]> {
  ...
  const selectedMediaItem = ref<{ category: AssetCategory; item: NexiaAssetItem } | null>(null)

  function selectMediaItem(category: AssetCategory, item: NexiaAssetItem): void {
    if (!item?.url) return
  ...
  function getMediaArray(cat: AssetCategory | null): NexiaAssetItem[] {
    const target = getTargetRef(cat)
    return target?.value ?? []
  }
