<template>
  ...
  >
    ...
    <div class="nexion-card-node__body" :class="{ 'nexion-card-node__body--selected': selected }" />

    <footer class="nexion-card-node__footer">
      <template v-if="showDetail">
        <div
          class="nexion-card-node__footer-line"
          :class="{ 'nexion-card-node__footer-line--stack': footerUsesStack }"
...
const showDetail = computed(() => {
  ...
  return store.showCardFooterDetail(props.id, viewportZoom.value)
})

/** 선택 시 Link ID·메타를 한 줄에 잘리지 않게 */
const footerUsesStack = computed(() => !!props.data.nestedInCard || props.selected)

/**
 * cqi 는 플로 좌표 박스 크기만 보므로, 캔버스 줌만 키우면 “돋보기 확대”처럼 보임.
 * 뷰포트 줌에 맞춰 플로 공간에서도 글자 하한을 키워 실제로 읽을 수 있게 함.
 */
/** 중첩 카드: 설정·cqi·하한을 함께 쓰도록 `--nxn-nested-*-floor` 만 주입. 선택 시에도 동일(인라인으로 fs-local 을 덮어쓰면 박스 대비 폰트만 과대해짐). */
const cardReadabilityStyle = computed(() => {
  ...
  let ramp = Math.sqrt(mul)
  if (props.selected) ramp *= 1.08
...
/** 뷰포트 줌으로 노드가 스케일될 때 화면상 글자가 설정 px 을 넘지 않도록 상한용(플로 공간에서는 ÷ 줌) */
const mergedCardStyle = computed(() => {
  const z = Math.max(0.001, viewportZoom.value)
  const out = { '--nxn-vp-zoom': String(z) }
  const read = cardReadabilityStyle.value
  if (read) Object.assign(out, read)
  return out
})
...
/* 컨테이너 = 카드 박스. --nxn-vp-zoom(스크립트) 기준으로 화면상 폰트 ≤ 설정 px, cqi·하한과 min */
.nexion-card-node {
  container-type: size;
  container-name: nxn-card;

  /* 플로 공간에서의 설정 상한: 설정 px ÷ 뷰포트 줌 → 캔버스 스케일 후 화면 px ≈ 설정 */
  --nxn-title-max-flow: calc(var(--nxn-card-title-fs, 13px) / var(--nxn-vp-zoom, 1));
  --nxn-body-max-flow: calc(var(--nxn-card-body-fs, 12px) / var(--nxn-vp-zoom, 1));
  --nxn-footer-max-flow: calc(var(--nxn-card-footer-fs, 11px) / var(--nxn-vp-zoom, 1));

  --nxn-card-title-fs-local: min(var(--nxn-title-max-flow), max(6px, 8cqi));
  --nxn-card-body-fs-local: min(var(--nxn-body-max-flow), max(5px, 7.2cqi));
  --nxn-card-footer-fs-local: min(var(--nxn-footer-max-flow), max(5px, 6.6cqi));
  ...
  &--selected.nexion-card-node--nested,
  &--selected.nexion-card-node--nested-deep {
    box-shadow: 0 0 0 2px rgba(25, 118, 210, 0.25), 0 2px 8px rgba(0, 0, 0, 0.08);
  }
}
...
.nexion-card-node--selected.nexion-card-node--nested .nexion-card-node__header,
.nexion-card-node--selected.nexion-card-node--nested-deep .nexion-card-node__header {
  min-height: 36px;
  padding: 6px 6px 6px 10px;
  gap: 4px;
}
...
.nexion-card-node--selected .nexion-card-node__title {
  white-space: normal;
  overflow: visible;
  text-overflow: unset;
...
.nexion-card-node--selected.nexion-card-node--nested .nexion-card-node__body,
.nexion-card-node--selected.nexion-card-node--nested-deep .nexion-card-node__body {
  padding: 8px;
}

.nexion-card-node__body--selected {
  overflow-x: hidden;
  overflow-y: auto;
  min-height: 0;
}
...
.nexion-card-node--selected.nexion-card-node--nested .nexion-card-node__footer,
.nexion-card-node--selected.nexion-card-node--nested-deep .nexion-card-node__footer {
  padding: 6px 10px 8px;
}
...
.nexion-card-node--selected .nexion-card-node__footer-id,
.nexion-card-node--selected .nexion-card-node__footer-meta {
  white-space: normal;
  overflow: visible;
  text-overflow: unset;
}
...
.nexion-card-node--selected.nexion-card-node--nested .nexion-card-node__handle,
.nexion-card-node--selected.nexion-card-node--nested-deep .nexion-card-node__handle {
  width: 14px;
  height: 14px;
  min-width: 14px;
  min-height: 14px;
