      ...
      <q-card-section>
        <q-form class="column q-gutter-md" @submit.prevent="onSubmit">
          ...
          <q-btn
            ...
            class="full-width"
            :loading="loading"
            :disable="!email.trim() || !password || password.length < 8"
          />
        </q-form>
      </q-card-section>
      <q-card-section class="text-center q-pt-none">
        <router-link to="/login" class="text-primary text-body2">이미 계정이 있으신가요? 로그인</router-link>
      </q-card-section>
    </q-card>
  </q-page>
</template>
...
async function onSubmit() {
  error.value = ''
  if (!email.value.trim() || !password.value || password.value.length < 8) return
  loading.value = true
  try {
