...
export async function scanConfigFiles() {
  ...
  try {
    ...
    const configModules = await Promise.allSettled([
      import('src/config/devGuideConfig.js').then(m => ({ name: 'devGuideConfig', path: 'src/config/devGuideConfig.js', module: m })),
      import('src/config/documentConfig.js').then(m => ({ name: 'documentConfig', path: 'src/config/documentConfig.js', module: m })),
      import('src/config/sidebarRegistry.js').then(m => ({ name: 'sidebarRegistry', path: 'src/config/sidebarRegistry.js', module: m })),
      import('src/config/componentTaxonomy.js').then(m => ({ name: 'componentTaxonomy', path: 'src/config/componentTaxonomy.js', module: m })),
      import('src/config/componentCategories.js').then(m => ({ name: 'componentCategories', path: 'src/config/componentCategories.js', module: m })),
      import('src/config/fileTypes.js').then(m => ({ name: 'fileTypes', path: 'src/config/fileTypes.js', module: m })),
      import('src/config/url-state/urlStateConfig.js').then(m => ({ name: 'urlStateConfig', path: 'src/config/url-state/urlStateConfig.js', module: m })),
      import('src/diagram/config/diagramSettings.js').then(m => ({ name: 'diagramSettings', path: 'src/diagram/config/diagramSettings.js', module: m })),
      import('src/components/parts-management/config/viewModeSettings.js').then(m => ({ name: 'viewModeSettings', path: 'src/components/parts-management/config/viewModeSettings.js', module: m })),
    ])
...
export function searchSettings(allSettings, query) {
  ...
  return allSettings.filter(setting => {
    return (
      ...
      setting.path.toLowerCase().includes(lowerQuery) ||
      setting.category.toLowerCase().includes(lowerQuery) ||
      JSON.stringify(setting.data).toLowerCase().includes(lowerQuery)
    )
  })
}
