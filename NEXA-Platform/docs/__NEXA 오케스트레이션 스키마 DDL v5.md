...
| §1 ~ §2   | 귀속 32개 + **§1-5 UCL** + **Self facet·Empathy** / 비귀속·Capability·코일          |
...
32개 핵심 귀속 테이블(§1-3에 `project_self_facet_runtime` 포함) + **[UCL-04]·[UCL-07]** 실행 트랙(§1-5). 실행 트랙은 단순 로그가 아니라 **과거·미래 유영 시뮬레이터**(가상 분기·스냅샷 롤백·잔여 적합도)의 데이터 기반이 된다.
...
COMMENT ON COLUMN project_logs.nixie_feedback IS 'NIXIE 비언어 피드백(Jitter)·족보 강결합 JSON. `confidence_score` < `user_defined_threshold`일 때 필수: `error_token`(UCL 정규화 토큰 또는 파싱 실패 지점), `parser_version`, 선택 `pipeline_id`/`ir_stage`. `user_defined_threshold_snapshot`(당시 적용 임계). 캔버스는 이 행과 1:1로 빛의 떨림 연출.';
...
### 1-5. UCL 실행 트랙 (`execution_chains`, `execution_steps`, `execution_logs`)

**근거:** [NEXA-UCL-04] 실행 사슬 생명주기·VOID, [NEXA-UCL-07] 예외·`ADAPTER_PARTIAL_SUCCESS`·스냅샷 롤백.
...
- **상태 스냅샷:** `pre_state_snapshot` / `post_state_snapshot` — 스텝 경계의 직전·직후 상태 박제. 복잡한 취소 로직 없이 [UCL-07] **타임머신 롤백**에 사용한다.
- **잔여 태스크 적합도:** `execution_chains.residual_fit_score` + `residual_fit_rationale` — 부분 성공 시 남은 단계 강행 여부 판단(UCL-07 §1.1, 80% 임계)의 **수치·근거**를 사슬 단위에 보관한다.
...
COMMENT ON COLUMN execution_chains.residual_fit_score IS '[UCL-07 §1.1] ADAPTER_PARTIAL_SUCCESS 등 부분 성공 후 잔여 스텝이 what_intent 달성에 적합한지 0~100. NULL이면 미평가.';
...
COMMENT ON COLUMN execution_steps.is_virtual IS 'true=Dry-run·시뮬레이션 분기. 실물 어댑터/EFF 미발동 [UCL-04].';
...
COMMENT ON COLUMN execution_steps.post_state_snapshot IS '스텝 직후 상태 박제. 타임머신 뒤로가기·즉시 롤백 [UCL-07 §3].';
...
- **UCL 실행 트랙:** `execution_steps.is_virtual=true` 인 스텝은 실물 어댑터/EFF를 호출하지 않는다. `target_entity_type` 으로 PHYSICAL/VIRTUAL/NEXU 를 강제하며, Dry-run과 본 실행은 동일 스키마에 **행 단위로** 분리한다.
- **스냅샷/롤백:** `pre_state_snapshot`·`post_state_snapshot` 은 스텝 경계의 상태 박제. STUCK·복구 시 [UCL-07] 에 따라 이전 `post_state_snapshot` 적용으로 롤백한다.
- **잔여 적합도:** `ADAPTER_PARTIAL_SUCCESS` 시 `execution_chains.residual_fit_score`·`residual_fit_rationale` 을 갱신하고, [UCL-07 §1.1] 80% 임계로 FLOW 유지 vs STUCK+ASK 를 분기한다.
