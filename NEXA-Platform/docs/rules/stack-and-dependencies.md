...
**온라인 미니 닉시(격자 HUD):** 연출·GSAP·임시 컨트롤 상세는 `docs/Nexion/[NXN] [SPEC] 심플 닉시 GSAP 적용 UI 구현 v0.1.md`. Nexnap·Pinia 계약은 `[NXN] [SPEC] 심플 닉시(Simple NIXIE) 테스트 버전 구현 v0.1.md`. **도트·정보 연출은 GSAP(Timeline)** 를 1차로 두며, **Rive**는 해당 HUD에서 필수 아님.

| 기술                     | 용도                                                         | 주의                                                                   |
| :----------------------- | :----------------------------------------------------------- | :--------------------------------------------------------------------- |
| **GSAP**                 | 미니 닉시 **도트 그리드·Timeline**·DOM/SVG/Canvas 트윈       | `package.json` 도입 시 본 표·NIXIE v0.2 명세 갱신; 번들 분리·lazy 검토 |
| **Rive**                 | (선택) 닉시 **표정·비언어적 서사** 등 벡터 상태 머신         | Web/런타임 통합 시 번들·성능·에디터 파이프라인 정의                    |
...
| 그룹            | 패키지                                                                   | 역할                             | 주의                                                                                                                              |
| :-------------- | :----------------------------------------------------------------------- | :------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------- |
| **HTTP**        | `express`, `cors`                                                        | API·CORS                         | `anchor_id` / `path_id` 기반 접근 (`coding-style.md`)                                                                             |
...
| 올바른 구분                                | 잘못된 혼동                        | 메모                                                                                     |
| :----------------------------------------- | :--------------------------------- | :--------------------------------------------------------------------------------------- |
| **Nexnap** — **NEXA Meta Action Protocol** | 일반 "맵"·다른 MAP 약어            | 사람의 의도를 기계 동작으로 잇는 **메타 액션 프로토콜**                                  |
| **Task Packet** (작업 패킷)                | 임의 JSON blob                     | Nexnap 실행 단위로서 **구조·검증(HEXAGON 등)이 정의된 패킷** — 용어집·Nexnap 명세와 정합 |
