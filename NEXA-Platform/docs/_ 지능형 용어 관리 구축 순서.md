...
본 문서는 Glossary 구축을 실제 작업 순서로 실행하기 위한 가이드다.  
기준 문서:

- `_ 지능형 용어 관리 CRUD 테이블 명세서.md`
- `_ 지능형 용어 관리 자산 통합 스키마 DDL.md`
- `_ 지능형 용어 관리 시스템 NEXA Glossary DB.md`
...
- `nexa_glossary_definitions` 생성
- `nexa_term_tokens` 생성 + `layer_type` CHECK
- `nexa_glossary_references` 생성 + `capability_id NOT NULL`
- `nexa_glossary_vectors` 생성 + HNSW 인덱스
- `nexa_glossary_audit_logs` 생성 + hypertable 적용
