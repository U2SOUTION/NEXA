...
- **project_orchestra (악보/태스크)**
  ...
  - **Nexnap(NEXA Meta Action Protocol)**: 사람이 쓴 시방서를 AI가 해석하여 `project_orchestra`에 **Nexnap 포맷의 실행 스크립트**로 박제한다.
...
- **project_logs (실행 이력)**
  - 컨트롤러 조작 시 발생한 로그는 **why_causality**에 **GOVERN(운영 규범)** 태그를 달고, **why_chain** JSONB에 "시방서 ID(Asset-ID)의 규칙에 따라 실행됨"을 명시한다. **why_chain** 구조: 인과 사슬로서 `inputs`(신호 ref_id), `reasoning`(로직/판단 근거), `effects`(액션·표정·Nexnap 후보) 노드로 구성. [NEXU-SCHEMA] 넥슈 데이터 스키마 참조.
...
| 실행 | `project_scripts`, `project_devices`, **project_panels**·**project_orchestra** | 시방서 기반 Nexnap·task_sequence에 따라 컨트롤러가 기기 구동. 사용자 WILL·시방서 RULE 대조. |
