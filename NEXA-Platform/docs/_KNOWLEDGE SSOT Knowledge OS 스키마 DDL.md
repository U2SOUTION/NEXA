...
**범위:** 본 DDL은 위 네임스페이스 중심이다. 엣지·실행·대화 등 **다른 지식 축**은 오케스트레이션 등 별도 SSOT를 보며, 개괄은 `_KNOWLEDGE ARCH 지식 운영체제(NEXA-OS) 운영 아키텍처.md` **§0** 참고.

**마무리 수준·고도화:** Knowledge DDL·계약은 본 문서에서 고정하고, 오케스트레이션·AI 런타임 세부는 **별도 SSOT·도메인**에서 확장한다. 문장 정리는 `_KNOWLEDGE REF *` **§1-B**를 본다.
...
CREATE TABLE IF NOT EXISTS nexa_knowledge_distribution_profiles (
  ...
  status SMALLINT NOT NULL DEFAULT 1,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT chk_knowledge_distribution_profile_name CHECK (profile_name IN ('nano', 'micro', 'kinetic','zenith')),
  CONSTRAINT chk_knowledge_distribution_tier CHECK (intelligence_tier IN ('nano', 'micro', 'kinetic','zenith')),
  ...
  CONSTRAINT chk_knowledge_distribution_vista_policy
    CHECK (intelligence_tier <>'zenith' OR max_payload_kb <= 4096),
...
COMMENT ON TABLE nexa_knowledge_traceability_paths IS 'N-PATH식 논리 경로 ↔ anchor_id(문서·실행 앵커 UUID)';
...
CREATE INDEX IF NOT EXISTS idx_doc_sync_anchor_id
  ON nexa_knowledge_doc_sync_state(anchor_id);
...
- **구형 스키마 마이그레이션:** 이미 배포된 DB에 `nexa_knowledge_traceability_paths.anchor_id` 또는 구형 `nexa_knowledge_doc_sync_state`(`id`,`doc_ref_path`,`last_hash` 등)가 있으면 **단일 마이그레이션**으로 컬럼 rename·신규 컬럼 추가·데이터 백필·인덱스 재생성을 수행한다. N-PATH 전개 컬럼(`project_id`,`link_id`,`physical_path` 등)은 **`[NXN] [DDL] NEXA Nexion 독립형 인덱스 및 자산 관리 DDL.sql`** §1-A를 참고해 `ALTER`로 확장한다(통합 SSOT 본 블록은 **경량 traceability** 정의를 유지할 수 있음).
