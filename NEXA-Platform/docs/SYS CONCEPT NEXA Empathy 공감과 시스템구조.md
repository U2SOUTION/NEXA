...
| 경계 | 출력 생성 전 Empathy 가드레일 검사 | UCL 0단계 |
| 응답 | VI 임계값에 따른 system prompt 자동 변경 | 에이전트 UCL |
...
카드 하나를 고르는 것만으로 코일 프리셋 + Empathy 임계값이 자동 적용됩니다.
...
사용자는 카드 하나를 클릭했을 뿐 인데. 내부에서는 HEXAGON 토큰 재구성 + 코일 재조정 + 에이전트 전환이 동시에 일어납니다.

---

### 막혔을 때 카드가 나타나는 조건 — 감지 로직
