...
**Yjs**는 문서의 **'내용'**을 동기화하지만, 구축 중인 **NFS(지능형 서사 파일 시스템)**는 그 문서의 **'정체성과 족보'**를 관리합니다.
...
이 구조는 물리적인 데이터 흐름뿐만 아니라, 시스템이 어떻게 사유하고(Coil), 어떻게 공감하며(Empathy), 어떻게 지식을 관리하는지(NFS/residency)를 한눈에 보여줍니다.
...
graph TD
    ...
    subgraph Intelligence_Brain [지능형 판단 레이어: Context & Decision]
        ...
        ASK -- "승인 시 (WILL)" --> EC[Execution Chain / UCL Bundle]
    end
    ...
    subgraph Knowledge_OS [지식 운영체제: Knowledge OS]
        ...
        NFS[(NFS 인덱스: Traceability Paths)]
...
3. **실행 레이어 (Execution):**
   ...
   - **Execution Chain:** UCL 패킷 단위로 실행 사슬을 형성하며, **is_virtual** 플래그를 통해 실물 기기에 영향을 주지 않는 시뮬레이션을 분리합니다.
   - **Adapter Execution:** 논리 명령을 실제 기기 API로 번역하여 연주하며, 실패 시 에러 토큰을 피드백 루프로 환류합니다.

4. **Knowledge OS 레이어:**

   - **Traceability Paths (NFS):** 물리적 경로와 불변의 `doc_anchor`를 연결하는 **Inode 인덱스**로, 지식의 족보를 유지합니다.
...
2. **데이터의 입체적 관리 (Knowledge OS):** PG-Vector(의미), JSONB(원본), NFS(추적), Residency(상주 원장)로 휘발성·영속성·맥락을 나눈다. Redis(Semantic Cache)는 최전방의 단기 기억 역할을 한다.
