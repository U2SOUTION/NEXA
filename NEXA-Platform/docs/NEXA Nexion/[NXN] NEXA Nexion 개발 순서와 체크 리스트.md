...
철학·부담 분리는 `[NXN] [CNCP] NEXA Nexion 지식 OS 관리 및 악보 설계 철학.md` **§1·§1.1**과 정합한다. **`project_id` 필수(DDL·API) = 테넌트 격리; Nexion ≠ 워크플로 프로젝트 도구** — 구현 시 **CNCP §1.2**, **API §2.2.1**, **SCHM §2.1**을 먼저 읽는다. 스키마·API·Phase는 아래 **두 티어**로 고정한다. 상세 매핑은 `[NXN] [SCHM] NEXA Nexion 독립형 인덱스 및 자산 관리 스키마.md` **§2.2**, API는 `[NXN] [API] NEXA Nexion API 및 통신 규약.md` **§1.1**을 본다.
...
- **Phase 3:** Doc Sync Crawler — NFS 인덱스·`doc_sync_state` 정합.
...
- **[ ] 파일 시스템 스캔:** `DOCS_PATH`·`NEXA-Documentation/` 등(`[NXN] [ARCH] NFS ...` §1.1).
...
- [ ] **진행 순서:** **PRD §3.2** — UI(1) → DB Tier A(2) → Crawler(3) → Late Anchoring(4) → Ext; **Phase 2 초반**에 NFS·Core API 정합 고정. 오케스트레이션·Tier B 전체는 **Phase B-ops**·배포 단계로 후행.
