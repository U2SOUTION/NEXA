...
본 문서는 **NEXA Nexion**을 VSCode/Cursor 확장 프로그램 형태로 운영할 때 필요한 핵심 기능을 정의한다.  
...
호출 주체는 Nexion 확장이지만, 엔진 계약은 NEXA 문서군 전체(NXN/UCL/KNOWLEDGE/AI 등)에서 공통으로 사용한다.
...
- 도메인별 프롬프트는 분리하되(예: NXN/UCL), 엔진 인터페이스는 단일 계약으로 유지한다.
...
모든 오류는 사용자 메시지와 함께 내부 로그에 기록하고, 필요 시 UCL 표준 `error_token` 매핑 레이어로 전달한다.
...
이 문서를 기준으로 이후 실제 구현 문서(ARCH/API SPEC)를 분리한다.
