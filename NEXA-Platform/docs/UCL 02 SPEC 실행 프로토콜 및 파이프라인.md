...
| **사용자에게 보이는 요약·카드 문구·채팅 응답**             | **한국어(또는 UI 로캘)** | Summary 레이어만 번역·로캘라이즈 |
...
### 3.1 시나리오: 「지금의 나」카드 → HEXAGON 재구성 (질문지 없는 온보딩)

사용자가 별도 설문 없이 **「지금의 나」(Now 계열) 카드**를 한 번 클릭하면, 파이프라인은 아래 순서로만 진행한다.

1. **Listen:** 클릭 이벤트를 입력으로 수집; 카드에 매핑된 프리셋 ID(예: `now_persona_v3`)를 IR에 합친다.
2. **Awareness:** 현재 세션·프로젝트·디바이스 상태와 Level 0~3 룰을 대조; 충돌 시 **STUCK**으로 전환해 ASK만 띄운다(긴 질문지 없음).
3. **Decision:** 카드가 의미하는 **의도 묶음**(예: “집중 모드”, “가벼운 대화”)을 `what_intent`·`why_causality`에 반영; 승인이 필요 없으면 **FLOW** 유지.
...
5. **Execution / Feedback:** `execution_chains`에 사슬 기록; 사용자에게는 **한국어 Summary**(카드 확인 문구 + 한 줄 요약)만 노출하고, 내부 IR은 영문 유지(2.0.1).
...
「지금의 나」시나리오(§3.1)와 동일한 메커니즘이 다른 카드(에너지·방향·발견)에도 적용 가능하다.
