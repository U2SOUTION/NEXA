...
| 뷰어 탭 | viewer | UniversalViewer | **모든 에디터 유형(문서·코드·이미지·오디오·영상)에 대응하는 멀티 뷰어**. 패널당 파일 1개. |
...
> **1단계 목표**: 모든 에디터를 `EditorPanel` 규격 인터페이스(Props/Events)로 추상화. 각 탭에서 **여러 패널(파일 인스턴스)** 을 동시에 열 수 있도록 구조화. EditorPanel 규격 상세는 별도 기술 문서에서 정의한다.
...
| 항목 | 저장소 | 키/위치 | 비고 |
|------|--------|---------|------|
| 스플릿 비율, 영역 표시/숨김, 탭 구성·열린 패널 목록·활성 패널·Focus Stack | localStorage | `nexa-ai-split-layout` | 1단계: **Pinia** 전환으로 탭·열린 패널·활성 패널·**Z-index·Focus Stack**·위치 정보 통합 관리 |
...
| **상태 관리** | **Pinia**를 사용하여 탭 구성, **열린 패널 목록**(탭별), 활성 패널 ID, 스플릿 뷰 위치 정보 저장. **패널 Z-index·Focus Stack** 정보도 함께 관리하여, AI 채팅에서 "방금 네가 수정한 그 코드"와 같은 **지칭의 정확도** 향상 |
...
| 항목 | 내용 |
|------|------|
| **컨텍스트 주입** | AI에게 질문할 때 **현재 활성화된 패널(열린 파일)** 의 데이터를 자동으로 포함하여 전송. **Focus Stack** 기반 최근 포커스 순서를 활용해 "방금 수정한 코드" 등 **지칭 지시(resolution)** 의 정확도 향상 |
...
| **페르소나·스킬·테스크 시스템** | **페르소나**(AI 역할·성향), **스킬**(기능), **테스크**(작업 단위)를 도메인·프로젝트별로 추가·관리·적용. **Vercel AI SDK**로 모델(Ollama·클라우드)을 교체해도 선택한 페르소나·스킬·테스크의 **일관성 유지** |
...
| 항목 | 내용 |
|------|------|
| **용어** | 워크스페이스(최상위), **탭**(editor/code/image/audio/video/viewer/chat/explorer, 스플릿 넘나들 수 있음), **패널**(탭 내 파일 1개 = 패널 1개), 배치 관리(QSplitter+Pinia) |
