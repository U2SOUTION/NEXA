...
### 2-2. 무한 확장을 위한 기술적 토대 (Nexnap 및 DB)
...
- **Nexnap 프로토콜 연동:** 확장된 코일들은 헥사곤 프로토콜의 **Why(인과)** 레이어와 결합됩니다. 24개의 코일이 있더라도 오케스트레이터는 이를 **CAUSE(원인), LOGIC(타당성), TARGET(최종목표)**이라는 3대 인과 토큰으로 수렴시켜 AI가 복잡한 추론 없이 1ms 내에 판단하게 합니다.
...
- **Ambient Interface:** 수많은 코일 슬라이더를 한꺼번에 노출하는 대신, 사용자가 선택한 **'지금의 나' Self facet (자아의 단면)**나 **Nexnap 템플릿**에 따라 필요한 코일 그룹만 자연스럽게 부각(Progressive Disclosure)됩니다.
...
| **Curiosity (호기심)**   | 새로운 자원 탐색 의지 | **[Nexnap-08]** 프로토콜에 따라 외부 API나 타 프로젝트 자원을 탐색하는 빈도 결정. |
...
> **코일 밸런서는 Nexnap의 부품이 아니라 NEXA 전체의 가치 조율 엔진입니다.**
