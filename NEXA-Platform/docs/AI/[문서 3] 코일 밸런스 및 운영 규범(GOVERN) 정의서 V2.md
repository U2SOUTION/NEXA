...
- **UCL 프로토콜 연동:** 확장된 코일들은 헥사곤 프로토콜의 **Why(인과)** 레이어와 결합됩니다. 24개의 코일이 있더라도 오케스트레이터는 이를 **CAUSE(원인), LOGIC(타당성), TARGET(최종목표)**이라는 3대 인과 토큰으로 수렴시켜 AI가 복잡한 추론 없이 1ms 내에 판단하게 합니다.
...
- **Ambient Interface:** 수많은 코일 슬라이더를 한꺼번에 노출하는 대신, 사용자가 선택한 **'지금의 나' 카드**나 **UCL 템플릿**에 따라 필요한 코일 그룹만 자연스럽게 부각(Progressive Disclosure)됩니다.
- **NIXIE 캔버스 연동:** 코일 밸런싱의 결과는 숫자가 아닌 **빛의 발광(Lumina)과 떨림(Jitter)**으로 표현됩니다. 예를 들어 가중치가 충돌하거나 확신도가 낮으면 빛이 미세하게 떨려 시스템의 '사유' 상태를 직관적으로 전달합니다.
...
| **Aesthetics (미학)**       | 시각적·형태적 가치        | **아트 도메인**에서 NIXIE 캔버스의 발광 강도와 프랙탈 분화의 화려함 결정.     |
...
| **Curiosity (호기심)**   | 새로운 자원 탐색 의지 | **[UCL-08]** 프로토콜에 따라 외부 API나 타 프로젝트 자원을 탐색하는 빈도 결정.   |
...
3.  **예상되는 주요 충돌 지점:**
    ...
    - **Stability <-> Creative:** NIXIE 캔버스의 도트들이 안정적으로 물리 좌표에 고정되어야 하는가, 아니면 논리적 관계에 따라 역동적으로 폭발해야 하는가?.
...
- **시각적 피드백:** 확신도가 임계값 미만일 경우 NIXIE UI의 빛이 미세하게 떨리며(Jitter) 시스템의 모호함을 사용자에게 전달한다.
...
```
proactivity=9  → 카드 자주·적극적으로 부상
proactivity=2  → 카드 거의 안 나타남
adaptivity=8   → 상황별로 다른 카드 종류 제시
```
...
> **코일 밸런서는 UCL의 부품이 아니라 NEXA 전체의 가치 조율 엔진입니다.**
