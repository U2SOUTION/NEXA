...
> **용어 기준**: [NEXA-STACK-01] §8.0

- **Workspace**: **UI 개념**만 사용. 각 도메인에서 사용되는 **추상적 작업공간**. 도메인당 작업 공간으로 표시 (예: AI 워크스페이스, BORDE 워크스페이스). **DB 테이블 없음**. 일부 도메인은 워크스페이스 개념을 쓰지 않아도 됨.
- **Project**: **DB 테이블 유지**. **최상위 작업단위**. 작업 분류·저장·불러오기 단위. 대부분 도메인과 밀접 연동.
