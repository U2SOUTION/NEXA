...
## 플랫폼 문서 3축과 권장 읽기·작업 순서

Nexion을 포함한 플랫폼 문서는 **한 덩어리 SSOT로 통합하지 않고**, 아래 세 축으로 **역할만 분리**한다. 기본틀을 잡을 때는 아래 **한 줄 순서**를 따르면 된다.

> **암기용 한 줄:** (((( `철학` → `PRD/UIUX` → `KNOWLEDGE 규격` → `오케스트레이션` ))))

| 축                            | 위치(예시)                             | 역할(한 줄)                                                                       |
| ----------------------------- | -------------------------------------- | --------------------------------------------------------------------------------- |
| **`[NEXA] *`**                | `docs/Nexion/` 등 (`[NEXA] [OS] …` 등) | **왜·원칙·서사**: NEXA-OS 철학, 제품 서사, 용어 뼈대                              |
| **`_KNOWLEDGE *`**            | `docs/_KNOWLEDGE*.md`                  | **`nexa_knowledge_*` SSOT**: 테이블·필드·DDL·CRUD SPEC 등 스키마·운영 규약의 진실 |
| **`__NEXA 오케스트레이션 *`** | `docs/__NEXA 오케스트레이션*` 등       | **실행·프로젝트·큐**: Tier B, RLS, 배포·실행 체인·테넌트 맥락                     |

| 순서 | 무엇을             | 대표 문서·폴더                                                                                                   |
| :--: | ------------------ | ---------------------------------------------------------------------------------------------------------------- |
|  1   | **철학·개념**      | `[NEXA] [OS] …`, `[NXN] [CNCP] …` — 구현 세부가 바뀌어도 방향을 고정                                             |
|  2   | **PRD / UI·UX**    | `[NXN] [PRD]`, `[NXN] NEXA Nexion 개발 순서와 체크 리스트`, `[NXN] [UIUX]` — **스프린트는 여기부터 진행해도 됨** |
|  3   | **KNOWLEDGE 규격** | `_KNOWLEDGE` SCHM·DDL·SPEC — 화면·API가 붙는 데이터의 **스키마 진실**                                            |
|  4   | **오케스트레이션** | `__NEXA 오케스트레이션*` — Nexion 코어의 **선행 조건로 두지 않음**(Tier B·연동·배포 단계에서 맞춤)               |

**UI/UX를 먼저 할 때:** Phase는 PRD·체크리스트와 동일하게만 진행하고, 화면이 가정하는 데이터가 **`_KNOWLEDGE`**인지 **`__NEXA`**(프로젝트·실행)인지 **한 줄 메모**만 남기면 이후 DDL 정리 비용이 줄어든다.
...
**Nexion ≠ 플랫폼 `project`:** `project_id`·`project_members`는 통합 DB·RLS의 **테넌트 구획**이지 Nexion 제품 정의의 중심이 아니다. 확장(Extension) 등은 **별도 프로그램**이다. 상세는 `[NXN] [CNCP] NEXA Nexion 지식 OS 관리 및 N-PATH(지도) 설계 철학.md` **§1.1**.
...
**Nexion 폴더 안 읽기 계층(비중):** 플랫폼 전체 3축·순서는 위 **「플랫폼 문서 3축과 권장 읽기·작업 순서」**를 본다.

| 계층           | 문서(예시)                                                                                                           | 용도                                                                           |
| -------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| **본문(핵심)** | `[NXN] [PRD]`, `[NXN] [CNCP]`, `[NXN] [UIUX]`, `[NXN] [ARCH] N-PATH …`, `[NXN] [SCHM]`, `[NXN] [DDL]`, `[NXN] [API]` | PRD는 **진행·Phase SSOT**; 나머지는 제품 정의·스키마·API·N-PATH **직접 근거**. |
| **부록·참고**  | `NEXA 부록-협업 스택·큐·오케스트레이션 흐름·머메이드.md`, 플랫폼 `__NEXA 오케스트레이션 스키마 DDL v5` 등            | 오케스트레이션·협업 스택 **맥락**; 순서 4단계·Tier B·배포 시 맞춤.             |
...
- **`NXN` (Nexion):** 본 제품(지식 맵·Vue Flow·N-PATH 연동 데스크) 기획 문서의 기본 코드.
- **`KOS` (Knowledge OS):** 문서 **파일명 컨텍스트 코드**로, 지식 축·운영체제 층을 짧게 태깅할 때 쓴다. **개념·용어의 정식 명칭은 용어집 SSOT인 `NEXA-OS`(지능형 지식 운영체제)** 이며, 본문에서는 첫 언급을 **`NEXA-OS`(지식 운영체제, 문맥 코드 `KOS`)** 처럼 **병기**하는 것을 권장한다. **신규 문서·교차 링크는 `NEXA-OS` 또는 위 병기 형식으로 통일**한다.
- **`Nexnap` (NEXA Meta Action Protocol):** 악보 설계 및 파싱 로직 중심의 문서에 사용.
...
- `[NXN] [CNCP] NEXA Nexion 지식 OS 관리 및 N-PATH(지도) 설계 철학.md`
...
- `[NXN] [API] NEXA Nexion API 및 통신 규약.md` — REST v1, N-PATH·동기화·링크·본문·캔버스·용어 추출

**스키마와 직결되는 데이터 축:**

- **지능형 서사 경로 체계(N-PATH) 인덱스:** 물리 폴더 구조와 논리 카드 계층을 연결하는 `nexa_knowledge_traceability_paths` 테이블이 핵심이다. **Inode** 역할로 탐색기에서의 위치 변경을 앵커 ID로 추적한다.
...
- **`[NXN] [ARCH] N-PATH 보안 및 외부 자산 연동 설계서` §3:** Doc Sync Crawler·배치 잡 관련 **구현 체크리스트**(경로·동기화 무결성·거버넌스·UX). 스캔 주기·해시·Soft 처리 방향은 여기서 아키텍처 범위로 정리하고, 세부 계약은 API·오케스트레이션 SSOT와 맞춘다.
