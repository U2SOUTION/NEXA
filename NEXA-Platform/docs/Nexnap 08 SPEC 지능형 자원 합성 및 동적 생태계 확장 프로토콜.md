## **[NEXA-Nexnap-08] 지능형 자원 합성 및 동적 생태계 확장 프로토콜**

본 문서는 외부 API, 가상 디바이스, 타 프로젝트 자원을 실시간으로 탐색하고 연결하여 NEXA 플랫폼을 **'자가 증식하는 생태계'**로 확장하는 **성장 전략**을 정의한다.

**연계 문서:**

- [NEXA-Nexnap-07] 예외 처리 및 자가 회복 시나리오 — Nexnap-08은 Nexnap-07의 "동적 자원 차출 프로토콜"을 확장·구체화한다.
- [NEXA-Nexnap-04] 실행 사슬 생명주기·`execution_steps`, [NEXA-Nexnap-03] 의사결정 매트릭스, [NEXA-DDL-00/01] DB 스키마.
...
**핵심 원칙:** Nexnap-07은 **'안전한 생존'**을 보장하고, Nexnap-08은 이를 토대로 시스템이 **'무한히 확장'**할 수 있는 데이터 구조와 논리 토대를 마련한다. 공유·차용 시에도 **소유자 우선권**과 **대상 시스템 무결성**이 항상 보장된다(§4.4). 이 두 문서가 결합되어 NEXA는 스스로 치유하고 성장하는 살아있는 지능 체계가 된다.

---

### 0. 오케스트레이션 vs. 동적 네트워크 합성 (Distinction)

기존 오케스트레이션이 '정해진 악보를 연주하는 지휘'라면, Nexnap-08은 **'필요한 악기를 실시간으로 찾아 무대에 올리는 구성'**에 가깝다.
...
문제 발생 시 또는 성능 최적화가 필요할 때, 시스템은 다음 4단계를 거쳐 자원을 재구성한다. [Nexnap-07] §문제 발생 시 동적 자원 차출 프로토콜과 동일한 흐름이다. **전제:** 사용자 활력 지수(VI)가 임계값을 넘을 때만 가동하며, VI 이하일 때는 Low-Entropy 모드(§7.5)로 제한된다.
...
- [Nexnap-07] 제안: `project_network_topology`(No.21)에 장치 간 물리적 거리(GPS)·논리적 연결망을 저장하여 기초가 된다.
...
- [Nexnap-04] `execution_steps.target_entity_type`, `is_virtual` 필드 활용.

### 3.3 자원 예약 필드 (execution_steps 확장 제안)

[Nexnap-07]에서 제안한 `execution_steps` 확장 필드. DDL 확정 시 [NEXA-DDL-01]에 반영한다.
...
[NEXA-DDL-00] `capability_map`(라우트·API 경로 ↔ Capability ID 매핑) 외에, [Nexnap-07] 제안대로 **장치의 잠재 역량**을 정의하는 확장을 고려한다.
...
NEXA 플랫폼에는 이미 여러 '공유' 관련 전략이 정의되어 있다. Nexnap-08의 공유는 이들과 **보완 관계**이며, 정합을 위해 아래 비교·보강이 필요하다.

| 구분                   | 기존 공유 전략                                                                                           | Nexnap-08 공유 | 차이점                             | 보강 방향                                                                                                                                                     |
| ---------------------- | -------------------------------------------------------------------------------------------------------- | -------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **정적 템플릿**        | [DDL-00 §2.3] `orchestra_scores`로 페르소나·스킬·대시보드 **템플릿화** → 프로젝트에 **재적용**           | —              | 사전 정의·선택적 적용              | Nexnap-08은 **동적 합성**이므로, 템플릿은 '시드'로 활용. 합성된 자원 조합을 나중에 **새 템플릿으로 저장**하는 피드백 루프 추가                                |
...
| **오픈 마켓**          | [GUIDE §5, ADMIN-01 §4.6] 페르소나·스킬·태스크 **템플릿** 무료/유료 배포                                 | §4.2 동일      | 거래·배포 중심                     | 마켓에서 구매한 템플릿을 Nexnap-08 파이프라인의 **자원 탐색** 후보로 포함. `global_knowledge_base`·`orchestra_scores`와 Fit Score로 연계                      |
...
- **Nexnap-08**: 실시간·동적(문제 발생 시 자동 차출, 구독, 시뮬레이션 후 투입)

**보강 원칙:** 기존 공유는 Nexnap-08의 **입력 소스**(탐색 후보)가 되고, Nexnap-08 합성 결과는 기존 공유 체계로 **피드백**(새 템플릿·정책 등록)된다.
...
- **보강:** 마켓 자산을 Nexnap-08 **자원 탐색** 단계의 후보로 포함. Fit Score로 적합 템플릿 추천.
...
- **즉시 반영:** 끊기 요청은 **즉시** 적용된다. 차용 중인 실행 사슬이 있다면 `how_state=STUCK`으로 전이하고, Nexnap-07 자가 회복 루프(대안 실행·ASK)로 대응한다.
...
이 전략은 [NEXA-AUTH-01] 권한 모델·[Nexnap-07] 자가 회복 루프와 연동하여, 공유·차용이 **상호 신뢰**를 훼손하지 않도록 한다.
...
- **다중체(Multi-faceted Self)와의 결합:** 사용자가 '작업하는 나'에서 '창작하는 나'로 전환할 때, 필요한 외부 툴·API 네트워크를 **Ambient Nexnap** 환경으로 즉시 재구성한다.
...
| **템플릿 구조** | 입력(Nexnap params) → OpenAPI 파라미터 매핑 → HTTP 요청 → 응답 파싱 → Nexnap 형식 반환                                  |
...
- **에러 처리:** 어댑터 내부 실패는 [Nexnap-07] 에러 토큰(ADAPTER_TIMEOUT 등)으로 재포장.
...
| 기존 오케스트레이션    | Nexnap-08 동적 네트워크 합성                             |
| ---------------------- | -------------------------------------------------------- |
