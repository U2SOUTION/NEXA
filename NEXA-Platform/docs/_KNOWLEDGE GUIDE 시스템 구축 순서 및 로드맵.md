...
- `_KNOWLEDGE ARCH 지식 운영체제(NEXA-OS) 운영 아키텍처.md` (**§0** 지식 스펙트럼 · **§1-B** 안전 요약 · **§1-C** OS적 보강 5가지 · **§1-D** AI·LLM 입문)
...
- **“토큰만 줄인다”**로만 이해하면 일부 테이블(감사·경로·헬스)의 목적이 어긋난다. 넓게는 **속도·정확도·안전**까지 포함해 보면 된다. 요약 표는 `_KNOWLEDGE ARCH 지식 운영체제(NEXA-OS) 운영 아키텍처.md` **§1-D**를 본다.
...
- `definitions.zenith.summary` 기반 한국어 요약 문장 생성
- 상태/안전 문구 결합 템플릿 적용
...
- `Nano/Micro/Kinetic/Zenith` 프로파일별 추출 규칙 적용
...
- [ ] 문서 동기화 자동화 정상
- [ ] 차등 배포 프로파일(`Nano/Micro/Kinetic/Zenith`) 정상
