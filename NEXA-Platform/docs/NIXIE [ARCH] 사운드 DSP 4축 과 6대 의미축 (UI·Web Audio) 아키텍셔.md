...
- **출력 경로는 둘**: (1) **레이어 프로브** — `nixieSoundLayerAudio.ts` 전용 `AudioContext`, 지속 테스트 톤. (2) **모스 미리듣기** — `morseWebAudio.ts`, 타임라인 이벤트마다 오실 생성. 둘은 **컨텍스트 분리**(동시 재생 시 그래프 충돌 방지).
...
| 모스 재생 | `playMorseTimeline`, `setMorseSoundLayerParams`, `setMorseCarrierFrequencyHz` | `src/system/nixie/morseWebAudio.ts` |
...
|------|------------------------------|------|
| 긴장 | `tension01` | dit·캐리어 오프셋·패닝 등과 함께 쓰임(표 B) |
...
- **R1**: 긴장·활력 둘 다 높을 때 지터 보수(0.85배) 또는 dit 배율 하한(0.82) — 코드 주석·테스트 참고.
...
- **ditScale**: 긴장·활력·기계성·이질감 등 가중 곱, R1 하한 적용.
- **carrierOffsetHzFromTension**: 최대 120×`tension01`.
- **carrierUncannyOffsetMaxHz**: 이질감 기반 Hz 상한(호출부 랜덤/LFO).
...
- **dit·톤 변경**: dit 스케일 변화는 타임라인 재구성이 필요할 수 있음 — 개발 패널의 `watch` 정책과 HUD 문서와 맞출 것.
...
3. `nixieSoundLayerAudio`·`morseWebAudio`에서 소비 여부 결정  
4. 단위 테스트·개발 패널 슬라이더
