...
#### 10. 외부 도트의 인입 연출 — N-MAP-08 자원 합성의 NEXA NIXIE 표현

[NEXA-N-MAP-08] 지능형 자원 합성 및 동적 생태계 확장 프로토콜에 따라 **타 프로젝트**나 **외부 API** 자원이 연결될 때, NEXU 캔버스에서 이를 어떻게 시각화할지에 대한 서사적 연출 가이드이다.

- **트리거:** N-MAP-08 파이프라인의 [자원 탐색 → 시뮬레이션 → 실제 투입] 중 **실제 투입**이 결정된 시점.
...
- **연계:** [N-MAP-08] §4.4 자원 보호·소유자 주권, §3.5 자원 신뢰도(Resource Confidence). 편입되는 도트는 `resource_confidence`·`sharing_status`에 따른 색상·Jitter 등급을 반영할 수 있다.
