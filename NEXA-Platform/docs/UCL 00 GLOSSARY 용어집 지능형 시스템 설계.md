...
5. 실행 사슬(Execution Chain) 및 단계(Execution Steps)
   실행 사슬은 UCL Protocol이 생성하는 “실시간 실행 엔티티”이며, 단계는 “원자적 마디(Atomic Action)”다.
...
6. 상태 전이(State Lifecycle) — FLOW / STUCK / VOID (+ 확장)
...
| UX VOID         | 사용자의 망설임/변심이 감지되어 영감 모드 진입         | 넥슈가 UCL 템플릿 성운 펼침                |
| Data VOID       | 노후화된 이력/규칙을 지식화하고 로우 데이터를 비움     | 지식/아카이브 정리                         |

---

7. HEXAGON Protocol(핵사곤) 및 5W1H 골격
   핵사곤은 “데이터 패킷의 헤더”로서 사건/맥락의 본질을 6축으로 고정한다.

| 축    | 약어/의미                            | 예시(문서 기준)            |
| ----- | ------------------------------------ | -------------------------- |
| Where | 공간/영향 범위(Scope)                | CORE/FIELD/DOMAIN          |
...
9. UCL Layer vs UCL Protocol
   문서의 UCL은 2개 층으로 나뉜다.

| 구분         | 의미 및 역할                                                            |
| ------------ | ----------------------------------------------------------------------- |
| UCL Layer    | 의미/정책 계층(목표 해석, 상태 추적, 충돌 조정, 컨텍스트 축적)          |
| UCL Protocol | 전송/형식 계층(스키마, 필수 필드, 코드값, 라우트/채널, 왕복 인터페이스) |

---

10. Capability ID(기능 자격 ID) 및 권한/연결 규격
...
11. 신뢰도/적합도 점수(Confidence & Fit Scores)
    UCL-08에서 자원 합성의 안전성을 수치화하는 핵심 축이다.
...
| Resource Confidence          | 외부 자원 “주장 값”과 “실측 결과”의 오차를 반영한 신뢰도        | UCL-08 사전 평정 결과(권장 DDL: proposal/knowledge에 기록)                                               |
| pre-rating / pre_rating_runs | is_virtual=true에서 3회 이상 테스트해 오차를 산출하는 사전 평정 | UCL-08 §3.5                                                                                              |

---

12. 외부 자원 합성(동적 Substitution) 핵심 용어
...
15. NEXA-NIXIE 캔버스 표현 언어
    NEXA-NIXIE는 데이터/상태를 “서사적 시각 언어”로 투영하는 캔버스 체계다.

| 용어                           | 의미 및 역할                                                                            |
| ------------------------------ | --------------------------------------------------------------------------------------- |
| NEXA-NIXIE / NEXA-NIXIE CANVAS | HEXAGON×6-COILS 기반 지능형 서사 시각화 캔버스                                          |
...
| 외부 도트 인입(VOID→편입)      | UCL-08에서 외부 자원이 VOID 외곽에서 클러스터 폭발로 현재 좌표계로 편입되는 연출 가이드 |

---

16. 에러 코드 및 서사적 애니메이션 매핑(넥슈/Rive)
