...
5. NEXU 채널 입력은 `nexa_self_profiles`/`nexa_self_states`로 Self 상태를 해석
...
### 4.7 Self 공통 자산 정책 (NEXU 공용)

- `nexa_self_*`는 NEXU 전용 UI 테이블이 아니라 오케스트레이션도 공유하는 공통 자산 계층이다.
...
- NIXIE 캔버스는 노드별 `confidence_score`를 `project_settings.user_defined_threshold`(기본 95)와 비교한다.
