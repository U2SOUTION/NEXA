...
export async function createDevice(userId, { name, device_type, mac_address } = {}) {
...
export async function getDeviceById(deviceId, userId) {
  const { rows } = await pool.query(
...
export async function updateDevice(deviceId, userId, payload) {
  ...
  if (payload.is_active === false) await invalidateDeviceCache(device.token_hash)
...
export async function updateDeviceLastSeen(deviceId, { mac_address, ip_address } = {}) {
