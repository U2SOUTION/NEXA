...
**Phase 1: 플로팅 메뉴 시스템 (주입 UI 기본 기능 완료)**
- ✅ Shadow DOM 기반 플로팅 메뉴 삽입 (injectUI.js)
- ✅ 완전 투명 배경 처리 (Shadow DOM, iframe, Vue 컴포넌트)
- ✅ 세로 탭 메뉴 UI 구현 (MultiDirectionTabs.vue)
- ✅ 타이틀 및 서브 타이틀 (NEXA / U2 SOLUTION)
- ✅ 드래그로 위치 이동 및 위치 저장/복원 (localStorage)
- ✅ 타이틀 클릭으로 아이콘 토글 기능
- ✅ 클릭/드래그 구분 (5px 임계값)
- ✅ 활성 탭 우측 완전 정렬 및 스타일링
- ✅ 아이콘/라벨 스타일링 (크기, 방향, 간격)

### 🚧 진행 중인 작업

- Phase 1: 플로팅 메뉴 시스템 (주입 UI 기본 기능 완료, 테마 감지 미구현)
...
- **주입 UI 시스템**: Shadow DOM 기반 완전 투명 플로팅 탭 메뉴 구현 (옵션 2)
  - 타이틀(NEXA/U2 SOLUTION) 및 세로 탭 메뉴 주입
  - 드래그로 위치 이동 및 위치 저장/복원
  - 타이틀 클릭으로 아이콘 토글 기능
  - 완전 투명 배경 처리 (Shadow DOM, iframe, Vue 컴포넌트)
  - 활성 탭 우측 완전 정렬 및 pill 모양 스타일
...
-   [x] 환경별 레이아웃 컴포넌트 ✅ **완료** (2024-12)
    ...
    -   [x] 🔥 **플로팅 메뉴**: 사이트 DOM 영역에 오버레이 방식 삽입 ✅ **완료** (2024-12)
        -   [x] Shadow DOM 기반 스타일 격리 시스템 ✅ **완료** (injectUI.js)
        -   [x] CSS 충돌 방지 메커니즘 (고유 클래스명, 스타일 격리) ✅ **완료** (완전 투명 배경)
        -   [x] 사이트 정책 확인 시스템 (CSP, Shadow DOM 지원 등) ✅ **완료** (checkSitePolicy)
        -   [x] 자동 폴백 처리 (사이트 정책 불가 시 사이드바로 전환) ✅ **완료**
        -   [x] 플로팅 메뉴 위치/크기 조정 기능 ✅ **완료** (드래그로 위치 이동, localStorage 저장)
        -   [x] 탭 메뉴 UI 구현 ✅ **완료** (MultiDirectionTabs.vue - 세로 탭 메뉴)
        -   [x] 타이틀 토글 기능 ✅ **완료** (클릭으로 아이콘 표시/숨김)
        -   [x] 클릭/드래그 구분 ✅ **완료** (5px 임계값)
...
-   [x] Background 기본 구조 ✅ **완료** (background.js)
    -   [x] 🔥 **플로팅 메뉴 삽입 시스템** (사용자 선택 옵션) ✅ **완료** (2024-12)
    -   [x] Shadow DOM 기반 플로팅 메뉴 삽입 로직 ✅ **완료** (injectUI.js)
    -   [x] 사이트 정책 확인 시스템 (`SitePolicyChecker`) ✅ **완료** (checkSitePolicy 함수)
    -   [x] CSS 충돌 방지 메커니즘 ✅ **완료** (Shadow DOM + 완전 투명 배경)
    -   [x] 자동 폴백 처리 (사이트 정책 불가 시 사이드바로 전환) ✅ **완료**
    -   [x] 플로팅 메뉴 토글 기능 ✅ **완료** (타이틀 클릭으로 아이콘 표시/숨김)
    -   [x] 탭 메뉴 UI 구현 ✅ **완료** (MultiDirectionTabs.vue - 세로 탭 메뉴)
    -   [x] 타이틀 및 서브 타이틀 표시 ✅ **완료** (NEXA / U2 SOLUTION)
    -   [x] 드래그 기능 ✅ **완료** (타이틀 드래그로 상하 이동)
    -   [x] 위치 저장/복원 ✅ **완료** (localStorage 기반)
    -   [x] 완전 투명 배경 처리 ✅ **완료** (Shadow DOM, iframe, Vue 컴포넌트 전체)
    -   [x] 아이콘/라벨 스타일링 ✅ **완료** (아이콘 크기, 라벨 가로 방향, 간격 조정)
    -   [x] 활성 탭 스타일링 ✅ **완료** (우측 완전 정렬, pill 모양)
    -   [x] 클릭/드래그 구분 ✅ **완료** (5px 임계값으로 구분)
