...
-   `DevMenuSlider`: 개발 도구 메뉴 슬라이더 (문서 관리, 에러 트래킹, **스타일 가이드** 등)
...
<template>
    <div class="dev-sidebar">
        ...
        <!-- 스타일 가이드 헤더 및 리스트 (activeMenu === 'style-guide') -->
        <template v-else-if="activeMenu === 'style-guide'">
            <StyleGuideHeader
                :categories="styleGuideCategories"
                :search-query="styleGuideSearchQuery"
                :filter-category="styleGuideFilterCategory"
                :filter-tags="styleGuideFilterTags"
                :view-mode="styleGuideViewMode"
                @search-change="handleStyleGuideSearchChange"
                @filter="handleStyleGuideCategoryFilterChange"
                @tags-change="handleStyleGuideTagsFilterChange"
                @view-mode-change="handleStyleGuideViewModeChange"
                @refresh="handleStyleGuideRefresh"
                @open-settings="openSettings"
            />

            <!-- 스타일 가이드 리스트 -->
            <StyleGuideList :samples="styleGuideFilteredSamples" :search-query="styleGuideSearchQuery" :selected-sample="styleGuideSelectedSample" :view-mode="styleGuideViewMode" @sample-selected="handleStyleGuideSampleSelect" />
        </template>
    </div>
</template>

<script setup>
import StyleGuideHeader from "./dev-tools/style-guide/StyleGuideHeader.vue";
import StyleGuideList from "./dev-tools/style-guide/StyleGuideList.vue";
import { useStyleGuide } from "src/composables/dev-tools/useStyleGuide.js";

// 스타일 가이드 관리 (composable 사용)
const {
    categories: styleGuideCategories,
    filteredSamples: styleGuideFilteredSamples,
    selectedSample: styleGuideSelectedSample,
    searchQuery: styleGuideSearchQuery,
    filterCategory: styleGuideFilterCategory,
    filterTags: styleGuideFilterTags,
    viewMode: styleGuideViewMode,
    handleSearchChange: handleStyleGuideSearchChange,
    handleCategoryFilterChange: handleStyleGuideCategoryFilterChange,
    handleTagsFilterChange: handleStyleGuideTagsFilterChange,
    handleViewModeChange: handleStyleGuideViewModeChange,
    handleSampleSelect: handleStyleGuideSampleSelect,
    refresh: handleStyleGuideRefresh,
    loadCategories: loadStyleGuideCategories,
} = useStyleGuide();
</script>
```
...
**참고**: 테마 관리와 동일하게 `DevSidebar.vue`에서 직접 `StyleGuideHeader`와 `StyleGuideList`를 조건부 렌더링하므로, 별도의 통합 컴포넌트(`StyleGuideSidebar.vue`)는 필요하지 않습니다.
...
<template>
    <q-page class="development-page">
        <div class="q-pa-lg">
            <!-- 스타일 가이드 컨텐츠 -->
            <StyleGuideContent v-else-if="activeMenu === 'style-guide'" :sort-option="styleGuideSortOption" :search-query="styleGuideSearchQuery" :category-filter="styleGuideCategoryFilter" :view-mode="styleGuideViewMode" />
        </div>
    </q-page>
</template>
...
// 스타일 가이드 상태 (DevSidebar와 동기화)
const styleGuideSearchQuery = ref("");
const styleGuideCategoryFilter = ref(null);
const styleGuideSortOption = ref("category");
const styleGuideViewMode = ref("flat");

// 스타일 가이드 상태 변경 이벤트 핸들러
function handleStyleGuideSearchChange(event) {
    styleGuideSearchQuery.value = event.detail.query || "";
}

function handleStyleGuideCategoryFilterChange(event) {
    styleGuideCategoryFilter.value = event.detail.category || null;
}

function handleStyleGuideViewModeChange(event) {
    styleGuideViewMode.value = event.detail.viewMode || "flat";
}

onMounted(() => {
    // 스타일 가이드 이벤트 리스너 등록
    window.addEventListener("style-guide-search-changed", handleStyleGuideSearchChange);
    window.addEventListener("style-guide-filter-changed", handleStyleGuideCategoryFilterChange);
    window.addEventListener("style-guide-view-mode-changed", handleStyleGuideViewModeChange);
});

onUnmounted(() => {
    window.removeEventListener("style-guide-search-changed", handleStyleGuideSearchChange);
    window.removeEventListener("style-guide-filter-changed", handleStyleGuideCategoryFilterChange);
    window.removeEventListener("style-guide-view-mode-changed", handleStyleGuideViewModeChange);
});
</script>
```

#### 2.2 스타일 가이드 컨텐츠 컴포넌트

**위치**: `NEXA-Platform/src/components/dev-tools/style-guide/StyleGuideContent.vue`

**역할**: 스타일 가이드 메인 컨텐츠 영역

**구조**:

```
StyleGuideContent.vue
├── 샘플 라이브러리 섹션
│   ├── 샘플 그리드 (카드 형태)
│   ├── 샘플 필터 (카테고리, 태그)
│   └── 샘플 검색
...
```
NEXA-Platform/src/components/dev-tools/style-guide/
├── StyleGuideContent.vue              # 메인 컴포넌트
├── sections/
│   ├── SampleLibrarySection.vue       # 샘플 라이브러리 섹션
│   ├── SampleDetailView.vue           # 샘플 상세 뷰
│   ├── DesignDecisionsSection.vue    # 디자인 결정 요소 섹션
...
<template>
    <div class="dev-tools-panel">
        ...
        <q-scroll-area class="fit">
            <!-- 스타일 가이드 패널 (activeMenu === 'style-guide') -->
            <div v-if="activeMenu === 'style-guide'" class="accordion-wrapper">
                <q-expansion-item icon="style" label="샘플 관리" :model-value="styleGuidePanelExpanded" @update:model-value="styleGuidePanelExpanded = $event">
                    <StyleGuidePanel :selected-sample="styleGuideSelectedSample" :usage-count="styleGuideUsageCount" :usage-files="styleGuideUsageFiles" @tag-updated="handleStyleGuideTagUpdate" @file-clicked="handleStyleGuideFileClick" />
                </q-expansion-item>
            </div>
        </q-scroll-area>
    </div>
...
<script setup>
import StyleGuidePanel from "./dev-tools/StyleGuidePanel.vue";
// ... 기타 import

// 스타일 가이드 패널 상태
const styleGuidePanelExpanded = ref(true);
const styleGuideSelectedSample = ref(null);
const styleGuideUsageCount = ref(0);
const styleGuideUsageFiles = ref([]);

// 스타일 가이드 샘플 선택 이벤트 리스너
function handleStyleGuideSampleSelected(event) {
    styleGuideSelectedSample.value = event.detail.sample;
    // 사용 통계 로드 (향후 구현)
    // loadSampleUsage(event.detail.sample.id)
}

onMounted(() => {
    window.addEventListener("style-guide-sample-selected", handleStyleGuideSampleSelected);
});

onUnmounted(() => {
    window.removeEventListener("style-guide-sample-selected", handleStyleGuideSampleSelected);
});
</script>
```

#### 3.2 스타일 가이드 전용 패널 컴포넌트

**위치**: `NEXA-Platform/src/components/sidebars/right/dev-tools/StyleGuidePanel.vue`
...
window.dispatchEvent(
    new CustomEvent("style-guide-sample-selected", {
        detail: { sample: selectedSample },
    })
);

// 중앙 컨텐츠에서 수신
window.addEventListener("style-guide-sample-selected", (event) => {
...
// 왼쪽 사이드바에서 검색 변경
window.dispatchEvent(
    new CustomEvent("style-guide-search-changed", {
        detail: { query: searchQuery.value },
    })
);

// 중앙 컨텐츠에서 수신
window.addEventListener("style-guide-search-changed", (event) => {
...
// 왼쪽 사이드바에서 필터 변경
window.dispatchEvent(
    new CustomEvent("style-guide-filter-changed", {
        detail: { category: filterCategory.value },
    })
);

// 중앙 컨텐츠에서 수신
window.addEventListener("style-guide-filter-changed", (event) => {
...
// 왼쪽 사이드바에서 뷰 모드 변경
window.dispatchEvent(
    new CustomEvent("style-guide-view-mode-changed", {
        detail: { viewMode: viewMode.value },
    })
);

// 중앙 컨텐츠에서 수신
window.addEventListener("style-guide-view-mode-changed", (event) => {
...
// 오른쪽 사이드바에서 태그 업데이트
window.dispatchEvent(
    new CustomEvent("style-guide-tag-updated", {
        detail: { sampleId: sample.id, tags: newTags },
    })
);

// 샘플 레지스트리 업데이트 및 UI 갱신
window.addEventListener("style-guide-tag-updated", (event) => {
...
window.dispatchEvent(
    new CustomEvent("style-guide-recent-samples-changed", {
        detail: { samples: recentSamples },
    })
);

// 왼쪽 사이드바에서 수신하여 UI 갱신
window.addEventListener("style-guide-recent-samples-changed", (event) => {
...
**위치**: `NEXA-Platform/src/modules/style-guide/services/sampleRegistry.js`
...
export const sampleRegistry = [
  {
    ...
    icon: 'bar_chart',
    componentPath: 'src/charts/NexaChart.vue', // 실제 컴포넌트 경로
    props: {                                    // 샘플 렌더링용 props
      type: 'bar',
      data: [...],
      options: {...}
...
export function getCategories() {
  ...
  return Array.from(categories).map(cat => ({
    ...
    count: sampleRegistry.filter(s => s.category === cat).length
  }))
}
...
**위치**: `NEXA-Platform/src/modules/style-guide/services/sampleManager.js`
...
**위치**: `NEXA-Platform/src/modules/style-guide/services/tagManager.js`
...
**위치**: `NEXA-Platform/src/modules/style-guide/services/sampleStorage.js`
...
const STORAGE_KEY_PREFIX = "style-guide-";
...
export function toggleFavoriteSample(sampleId) {
    try {
        ...
        // 전역 이벤트 발생
        window.dispatchEvent(
            new CustomEvent("style-guide-favorite-samples-changed", {
...
```
NEXA-Platform/src/modules/style-guide/
└── services/
...
```javascript
// 샘플 추가
import { sampleRegistry } from "src/modules/style-guide/services/sampleRegistry.js";
...
```javascript
import { searchSamples, filterSamples, sortSamples } from "src/modules/style-guide/services/sampleManager.js";
...
```javascript
import { addTagToSample, removeTagFromSample, getTagSuggestions } from "src/modules/style-guide/services/tagManager.js";
...
```javascript
import { addRecentSample, toggleFavoriteSample, updateSampleUsage } from "src/modules/style-guide/services/sampleStorage.js";
...
1. **sampleRegistry.js에 샘플 추가**:
    ...
    export const sampleRegistry = [
        // ... 기존 샘플들
        {
            ...
            icon: "gauge",
            componentPath: "src/panel/NexaPanel.vue",
            props: {
                /* ... */
            },
            codeSnippet: `...`,
...
1. **태그 추가**:
    ...
    ```javascript
    import { addTagToSample } from "src/modules/style-guide/services/tagManager.js";
...
2. **태그 제거**:
    ...
    ```javascript
    import { removeTagFromSample } from "src/modules/style-guide/services/tagManager.js";
...
3. **태그 자동완성**:
    ...
    ```javascript
    import { getTagSuggestions } from "src/modules/style-guide/services/tagManager.js";
...
-   [ ] `src/components/sidebars/left/dev-tools/style-guide/` 디렉토리 생성
-   [ ] `src/components/dev-tools/style-guide/` 디렉토리 생성
-   [ ] `src/components/dev-tools/style-guide/sections/` 디렉토리 생성
-   [ ] `src/components/dev-tools/style-guide/components/` 디렉토리 생성
-   [ ] `src/components/sidebars/right/dev-tools/` 디렉토리 확인 (이미 존재)
-   [ ] `src/composables/dev-tools/` 디렉토리 확인 (이미 존재)
-   [ ] `src/modules/style-guide/services/` 디렉토리 생성

#### 2단계: 기본 컴포넌트 파일 생성 (빈 템플릿)

-   [ ] `StyleGuideHeader.vue` 생성 (빈 템플릿)
-   [ ] `StyleGuideList.vue` 생성 (빈 템플릿)
-   [ ] `StyleGuideContent.vue` 생성 (빈 템플릿)
-   [ ] `StyleGuidePanel.vue` 생성 (빈 템플릿)
-   [ ] `useStyleGuide.js` 생성 (기본 구조)

#### 3단계: 가상 데이터 생성 (sampleRegistry.js)

-   [ ] `sampleRegistry.js` 생성 및 초기 샘플 데이터 추가 (5-10개 샘플)
-   [ ] 샘플 데이터 구조: `id`, `name`, `displayName`, `category`, `hierarchy`, `tags`, `description`, `icon`, `codeSnippet`

**가상 데이터 샘플 예시**:

```javascript
// src/modules/style-guide/services/sampleRegistry.js
export const sampleRegistry = [
    {
        id: "nexa-chart-bar",
        name: "넥사차트-막대그래프",
        ...
        description: "NexaChart 컴포넌트를 사용한 막대 그래프 샘플",
        icon: "bar_chart",
        componentPath: "src/charts/NexaChart.vue",
...
const chartOptions = {
  ...
  <NexaPanel type="gauge" :value="75" :max="100" />
    ...
    },
    {
        ...
        icon: "menu",
        componentPath: "src/components/sidebars/left/DevSidebar.vue",
        codeSnippet: `<template>
  <div class="dev-sidebar">
...
#### 4단계: 왼쪽 사이드바 UI 구현 (StyleGuideHeader.vue)
...
-   새로고침 버튼: 샘플 목록 새로고침
-   설정 버튼: 스타일 가이드 설정 (향후 구현)

#### 5단계: 왼쪽 사이드바 UI 구현 (StyleGuideList.vue)
...
#### 6단계: 중앙 컨텐츠 UI 구현 (StyleGuideContent.vue)
    ...
    <div class="style-guide-content">
        ...
        <div v-if="!selectedSample" class="sample-library-view">
            <div class="library-header">
                <h2 class="library-title">스타일 가이드 샘플 라이브러리</h2>
...
-   [ ] `DevMenuSlider.vue` 수정: 'style-guide' 메뉴 항목 추가
-   [ ] `DevelopmentPage.vue` 수정: `activeMenu === 'style-guide'` 조건부 렌더링 추가
-   [ ] `DevSidebar.vue` 수정: `activeMenu === 'style-guide'` 조건부 렌더링 추가
-   [ ] `DevToolsPanel.vue` 수정: `activeMenu === 'style-guide'` 조건부 렌더링 추가
...
-   [ ] `/dev` 페이지 접근 시 스타일 가이드 메뉴 표시 확인
-   [ ] 스타일 가이드 메뉴 클릭 시 올바른 컴포넌트 표시 확인
...
#### 왼쪽 사이드바 (StyleGuideHeader.vue + StyleGuideList.vue)
...
#### 오른쪽 사이드바 패널 (StyleGuidePanel.vue)
...
#### 12단계: Composable 구현 (useStyleGuide.js)
...
-   [ ] `NEXA-Platform/src/components/sidebars/left/dev-tools/style-guide/StyleGuideHeader.vue`
-   [ ] `NEXA-Platform/src/components/sidebars/left/dev-tools/style-guide/StyleGuideList.vue`

#### 중앙 컨텐츠 컴포넌트

-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/StyleGuideContent.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/SampleLibrarySection.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/SampleDetailView.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/DesignDecisionsSection.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/ColorSystemSection.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/TypographySection.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/SpacingSection.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/sections/ComponentStyleSection.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/components/SampleCard.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/components/SampleGrid.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/components/SampleFilter.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/components/TagManager.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/components/CodeSnippet.vue`
-   [ ] `NEXA-Platform/src/components/dev-tools/style-guide/components/SamplePreview.vue`

#### 오른쪽 사이드바 패널

-   [ ] `NEXA-Platform/src/components/sidebars/right/dev-tools/StyleGuidePanel.vue`

#### Composable

-   [ ] `NEXA-Platform/src/composables/dev-tools/useStyleGuide.js`

#### 서비스 모듈 (샘플 관리)

-   [ ] `NEXA-Platform/src/modules/style-guide/services/sampleRegistry.js` (핵심 데이터)
-   [ ] `NEXA-Platform/src/modules/style-guide/services/sampleManager.js` (검색, 필터, 정렬)
-   [ ] `NEXA-Platform/src/modules/style-guide/services/tagManager.js` (태그 관리)
-   [ ] `NEXA-Platform/src/modules/style-guide/services/sampleStorage.js` (localStorage 관리)

### 통합 체크리스트

#### 메인 페이지 통합

-   [ ] `DevelopmentPage.vue`에 스타일 가이드 통합 (조건부 렌더링)
-   [ ] 이벤트 리스너 등록/해제

#### 왼쪽 사이드바 통합

-   [ ] `DevSidebar.vue`에 스타일 가이드 통합 (조건부 렌더링)
-   [ ] `useStyleGuide` composable 사용
-   [ ] 이벤트 핸들러 연결

#### 오른쪽 사이드바 통합

-   [ ] `DevToolsPanel.vue`에 스타일 가이드 통합 (조건부 렌더링)
-   [ ] 이벤트 리스너 등록/해제
