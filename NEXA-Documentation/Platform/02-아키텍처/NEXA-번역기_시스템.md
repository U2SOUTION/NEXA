...
**작성일**: 2024년  
**목적**: Platform의 핵심 컴포넌트 간 통신을 위한 중간 계층 시스템 설계  
**버전**: 1.0  
**위치**: Platform 핵심 아키텍처
...
번역기 시스템은 **NEXA Platform의 핵심 컴포넌트들 간 통신을 가능하게 하는 중간 계층**입니다. 각 컴포넌트(노드, 보드, 차트, 블록, 패널, 티치, 다이어그램)는 자신의 성격에 맞게 최적화된 고유한 데이터 구조를 가지지만, 서로 다른 컨텍스트에서 재사용하기 위해서는 구조 변환이 필요합니다. 번역기 시스템은 이러한 변환을 자동화하여 컴포넌트 간 원활한 통신을 가능하게 합니다.

### 핵심 컴포넌트의 성격과 통신의 어려움

Platform에는 다음과 같은 핵심 컴포넌트들이 있습니다:

| 컴포넌트 | 성격 | 주요 특징 | 통신 어려움 |
|---------|------|----------|------------|
| **노드 (Node)** | 워크플로우/자동화 | 그래프 기반 실행 로직, 트리거-처리-액션 구조 | 실행 컨텍스트가 다름 |
| **보드 (Board)** | 대시보드/레이아웃 | 패널 기반 그리드 레이아웃, 드래그앤드롭 | 레이아웃 중심 구조 |
| **차트 (Chart)** | 데이터 시각화 | 차트 타입별 특화 구조, 데이터 중심 | 시각화 전용 구조 |
| **블록 (Block)** | UI 구성 요소 | 단순한 표시 단위, 재사용 가능한 위젯 | 독립적인 UI 컴포넌트 |
| **패널 (Panel)** | 보드 내 표시 단위 | 보드 컨텍스트 종속, 그리드 위치 정보 | 보드 전용 구조 |
| **티치 (Teach)** | 액션 녹화/재생 | 시퀀스 기반 액션 저장, 자동화 인프라 | 시간 기반 액션 구조 |
| **다이어그램 (Diagram)** | 데이터 구조 시각화 | ERD, 플로우차트, 네트워크 등 구조 표현 | 그래프 기반 구조, 노드-엣지 관계 |

**통신의 어려움:**

1. **구조적 차이**: 각 컴포넌트는 자신의 목적에 최적화된 고유한 데이터 구조를 가짐
2. **컨텍스트 차이**: 같은 데이터라도 사용되는 컨텍스트에 따라 필요한 정보가 다름
3. **인터랙션 차이**: 각 컴포넌트의 인터랙션 방식이 다름 (드래그앤드롭, 클릭, 실행 등)
4. **의존성 차이**: 일부 컴포넌트는 특정 환경에 종속적 (예: 패널은 보드에 종속)

### 번역기 시스템이 해결하는 문제

번역기 시스템은 다음과 같은 문제를 해결합니다:

1. **컴포넌트 간 재사용**: 노드의 실행 결과를 보드 패널로 표시, 차트를 문서에 삽입 등
2. **컨텍스트 변환**: 보드의 대시보드를 ERP 문서에 임베드, 블록을 패널로 변환 등
3. **인터랙션 변환**: 노드의 실행 로직을 보드 패널의 인터랙션으로 변환
4. **데이터 구조 변환**: 각 컴포넌트의 고유 구조를 표준 계약 기반으로 변환
...
4. **컨텍스트 보존**: 원본 컴포넌트의 의도와 기능을 최대한 보존하면서 변환
...
번역기 시스템은 Platform의 핵심 아키텍처 구성 요소입니다:
...
1. **통신 계층**: 컴포넌트 간 데이터 교환을 위한 중간 계층
2. **변환 엔진**: 각 컴포넌트의 고유 구조를 다른 컨텍스트에 맞게 변환
3. **표준화 계층**: 표준 계약을 통해 예측 가능한 변환 제공

---

## 참고 문서

- [NEXA-컴포넌트_표준_계약.md](../NEXA-Documentation/Platform/04-개발/NEXA-컴포넌트_표준_계약.md): 표준 계약 정의
- [NEXA-인터랙션_표준_안내.md](./NEXA-인터랙션_표준_안내.md): 인터랙션 표준 상세 규격
- [NEXA-컴포넌트_재사용_가이드.md](../NEXA-Documentation/Platform/04-개발/NEXA-컴포넌트_재사용_가이드.md): 컴포넌트 재사용 가이드
- [NEXA-ARCHITECTURE.md](../NEXA-Documentation/Platform/02-아키텍처/NEXA-ARCHITECTURE.md): Platform 전체 아키텍처
