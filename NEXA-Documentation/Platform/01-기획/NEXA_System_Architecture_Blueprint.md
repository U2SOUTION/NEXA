...
-   **BOARD (사용자 캔버스):** `PANNEL`에서 가져온 요소들을 자유롭게 배치하는 공간입니다. 사용자는 자신만의 대시보드를 직접 디자인하며, 이후 설명할 `NODE`나 `TEACH`에서 생성된 복잡한 로직 결과물도 이곳에 배치하여 실시간으로 모니터링하고 제어합니다.

## ✨ 2. NODE & TEACH (지능형 로직 생성)
...
-   **TEACH (행위 학습 자동화):** 사용자의 실제 조작 패턴을 학습합니다. '녹화'를 누르고 보드에서 수행한 일련의 제어 동작을 데이터화한 뒤, 여기에 **단축키, 예약 시간, 특정 조건(예: 일산화탄소 농도 0.01% 이상 시)** 등을 부여하여 정교한 자동 실행 시나리오를 완성합니다.
...
3.  **자동화 학습 (TEACH):** "배터리 전압이 12V 이하로 떨어지면 냉장고를 절전 모드로 전환"하는 동작을 녹화하고 조건을 부여하여 자동화합니다.
...
`NEXA NODE`와 `NEXA TEACH`의 핵심 엔진으로, 복잡한 시스템의 논리적 흐름을 정의합니다.
...
-   **상태 레코딩 (TEACH 연동):** 다이어그램 내에서 발생하는 모든 상태 변화는 JSON 형태로 직렬화되어 `TEACH` 모듈에 저장될 수 있는 구조를 가져야 합니다.
...
└── logic/                   # 지능형 서비스 관련 (NODE, TEACH 대응)
...
4. **TEACH (`logic/teach-record.ts`):** 사용자가 직접 창문을 닫는 속도를 녹화하여 자동화 시나리오를 완성합니다.
...
`TEACH` 메뉴에서 녹화되는 사용자의 동작은 단순한 '좌표 값'이 아니라 '의도된 행위'로 저장되어야 합니다.
...
-   **자연어-노드 변환 (Natural Language to Node):** "오늘 밤은 배터리를 아끼면서 실내 온도를 20도로 유지해줘"라는 명령을 받으면, AI가 `NODE` 메뉴에서 관련 센서와 액추에이터를 연결하고 `TEACH`에 저장된 최적 패턴을 불러오는 구조입니다.
...
```mermaid
    ...
    participant Teach as TEACH (녹화/의도태깅)
...
# ✨ NEXA 시스템 구축을 위한 Zod 기반 로드맵 및 체크리스트

지금까지 설계한 아키텍처를 실질적인 코드로 구현하기 위한 단계별 실행 계획입니다. 이 로드맵은 데이터의 최소 단위인 **기초 규격**부터 시작하여, UI와 로직이 결합된 **동적 모듈** 순으로 진행됩니다.
...
가장 먼저 시작해야 할 작업은 **`common/metadata.ts`**와 **`common/semantic.ts`**의 코드를 작성하는 것입니다. 이 파일들이 있어야 이후의 패널과 노드 스키마들이 이를 상속받아 확장할 수 있기 때문입니다.

**바로 `common/metadata.ts`의 Zod 코드 초안 작성을 시작해 볼까요?** 본인이 생각하시는 ID 체계(UUID 등)나 반드시 포함되어야 할 장치 정보가 있다면 말씀해 주세요!
